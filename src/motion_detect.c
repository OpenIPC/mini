...
HI_S32 IVE_Md_Init(SAMPLE_IVE_MD_S *pstMd, HI_U16 u16ExtWidth, HI_U16 u16ExtHeight, HI_U16 u16BaseWidth, HI_U16 u16BaseHeight);
HI_S32 DmaImage(VIDEO_FRAME_INFO_S *pstFrameInfo, IVE_DST_IMAGE_S *pstDst, bool instant);
HI_S32 IVE_CreateImage(IVE_IMAGE_S *pstImg,IVE_IMAGE_TYPE_E enType,HI_U16 u16Width,HI_U16 u16Height);
HI_S32 IVE_CreateMemInfo(IVE_MEM_INFO_S*pstMemInfo, HI_U32 u32Size);

int motion_detect_init() {
    HI_S32 s32Ret = HI_IVS_MD_Init();
    if (HI_SUCCESS != s32Ret) { printf("HI_IVS_MD_Init failed with %#x!\n%s\n", s32Ret, hi_errstr(s32Ret)); }
    return s32Ret;
}
...
int motion_detect_run(HI_VOID * pArgs) {
    ...
    if (HI_SUCCESS != s32Ret) { printf("HI_IVS_MD_CreateChn failed with %#x!\n%s\n", s32Ret, hi_errstr(s32Ret)); return EXIT_FAILURE; }
    ...
    s32Ret = IVE_Md_Init(&stMd, u16ExtWidth, u16ExtHeight, u16BaseWidth, u16BaseHeight);

    // SAMPLE_IVE_MD_S *pstMd = &pstMd;
    MD_ATTR_S *pstMdAttr = &(stMd.stMdAttr);

    while (keepRunning) {
        s32Ret = HI_MPI_VI_GetFrame(viExtChn, &stExtFrmInfo, s32GetFrameMilliSec);
        if (HI_SUCCESS != s32Ret) { printf("HI_MPI_VI_GetFrame failed with %#x!\n%s\n", s32Ret, hi_errstr(s32Ret)); }
        ...
        if (!first_frame) {
            ...
            if (HI_SUCCESS != s32Ret) { printf("DmaImage failed with %#x!\n%s\n", s32Ret, hi_errstr(s32Ret)); goto BASE_RELEASE; }
            // SAMPLE_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret, BASE_RELEASE, "SAMPLE_COMM_DmaImage fail,Error(%#x)\n",s32Ret);
        } else {
            s32Ret = DmaImage(&stExtFrmInfo,&stMd.astImg[1 - s32CurIdx], instant);
            if (HI_SUCCESS != s32Ret) { printf("DmaImage failed with %#x!\n%s\n", s32Ret, hi_errstr(s32Ret)); goto BASE_RELEASE; }
            // SAMPLE_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret, BASE_RELEASE, "SAMPLE_COMM_DmaImage fail,Error(%#x)\n",s32Ret);
            first_frame = false;
            goto CHANGE_IDX;
        ...
        s32Ret = HI_IVS_MD_Process(md_chn, &stMd.astImg[s32CurIdx], &stMd.astImg[1 - s32CurIdx], NULL, &stMd.stBlob);
        if (HI_SUCCESS != s32Ret) { printf("HI_IVS_MD_Process failed with %#x!\n%s\n", s32Ret, hi_errstr(s32Ret)); goto BASE_RELEASE; }
        ...
        if (HI_SUCCESS != s32Ret) { printf("HI_MPI_VI_ReleaseFrame failed with %#x!\n%s\n", s32Ret, hi_errstr(s32Ret)); }
...
int motion_detect_deinit() {
    ...
    if (HI_SUCCESS != s32Ret) { printf("HI_IVS_MD_Exit failed with %#x!\n%s\n", s32Ret, hi_errstr(s32Ret)); }
    return s32Ret;
}
...
HI_S32 DmaImage(VIDEO_FRAME_INFO_S *pstFrameInfo, IVE_DST_IMAGE_S *pstDst, bool instant) {
    ...
    s32Ret = HI_MPI_IVE_DMA(&hIveHandle, &stSrcData, &stDstData, &stCtrl, instant);
    if (HI_SUCCESS != s32Ret) { printf("HI_MPI_IVE_DMA failed with %#x!\n%s\n", s32Ret, hi_errstr(s32Ret)); return s32Ret; }
    ...
    if (instant) {
        ...
        if (HI_SUCCESS != s32Ret) { printf("HI_MPI_IVE_Query failed with %#x!\n%s\n", s32Ret, hi_errstr(s32Ret)); return s32Ret; }
    }
    return HI_SUCCESS;
}
...
HI_S32 IVE_Md_Init(SAMPLE_IVE_MD_S *pstMd, HI_U16 u16ExtWidth, HI_U16 u16ExtHeight, HI_U16 u16BaseWidth, HI_U16 u16BaseHeight) {
    ...
    for (uint32_t  i = 0; i < SAMPLE_IVE_MD_IMAGE_NUM; i++) {
        HI_S32 s32Ret = IVE_CreateImage(&pstMd->astImg[i],IVE_IMAGE_TYPE_U8C1,u16ExtWidth,u16ExtHeight);
        if (HI_SUCCESS != s32Ret) { printf("SAMPLE_COMM_IVE_CreateImage failed with %#x!\n%s\n", s32Ret, hi_errstr(s32Ret)); goto MD_INIT_FAIL; }
    }
    HI_U32 u32Size = sizeof(IVE_CCBLOB_S);
    HI_S32 s32Ret = IVE_CreateMemInfo(&pstMd->stBlob,u32Size);
    if (HI_SUCCESS != s32Ret) { printf("SAMPLE_COMM_IVE_CreateMemInfo failed with %#x!\n%s\n", s32Ret, hi_errstr(s32Ret)); goto MD_INIT_FAIL; }
    ...
    MD_INIT_FAIL:
    if(HI_SUCCESS != s32Ret) { IVE_Md_Uninit(pstMd); }
    return s32Ret;
}
...
HI_S32 IVE_CreateImage(IVE_IMAGE_S *pstImg,IVE_IMAGE_TYPE_E enType,HI_U16 u16Width,HI_U16 u16Height) {
    HI_U32 u32Size = 0;
    HI_S32 s32Ret;
    if (NULL == pstImg) { printf("pstImg is null\n"); return HI_FAILURE; }
    ...
    switch(enType) {
        ...
        case IVE_IMAGE_TYPE_S8C1: {
            ...
            if (HI_SUCCESS != s32Ret) { printf("HI_MPI_SYS_MmzAlloc failed with %#x!\n%s\n", s32Ret, hi_errstr(s32Ret)); return s32Ret; }
        }
            break;
...
HI_S32 IVE_CreateMemInfo(IVE_MEM_INFO_S*pstMemInfo, HI_U32 u32Size) {
    if (NULL == pstMemInfo) { printf("pstMemInfo is null\n"); return HI_FAILURE; }
    pstMemInfo->u32Size = u32Size;
    HI_S32 s32Ret = HI_MPI_SYS_MmzAlloc(&pstMemInfo->u32PhyAddr, (void**)&pstMemInfo->pu8VirAddr, NULL, HI_NULL, u32Size);
    if (HI_SUCCESS != s32Ret) { printf("HI_MPI_SYS_MmzAlloc failed with %#x!\n%s\n", s32Ret, hi_errstr(s32Ret)); return s32Ret; }
    return HI_SUCCESS;
}
...
do{\
	if ((0 != (phy)) && (NULL != (vir))) { \
		 HI_MPI_SYS_MmzFree((phy),(vir)); (phy) = 0; (vir) = NULL;\
	}\
}while(0)
