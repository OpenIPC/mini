...
void PrefsInit() {
    ...
    if (getdomainname(stPrefs.hostname + l + 1, sizeof(stPrefs.hostname) - l) !=
        0) {
        stPrefs.hostname[l] = '.';
    }
...
int SendDescribeReply(
    RTSP_buffer *rtsp, char *object, char *descr, char *s8Str) {
...
int send_setup_reply(
    RTSP_buffer *pRtsp, RTSP_session *pSession, RTP_session *pRtpSes) {
    char s8Str[1024];
    sprintf(
    ...
    case RTP_rtp_avp:
        if (pRtpSes->transport.u.udp.is_multicast) {
            ...
            sprintf(
                ...
                pRtpSes->transport.u.udp.cli_ports.RTP,
                pRtpSes->transport.u.udp.cli_ports.RTCP, "192.168.245.96");
        }

        sprintf(
            s8Str + strlen(s8Str), "%d-%d" RTSP_EL,
            pRtpSes->transport.u.udp.ser_ports.RTP,
            pRtpSes->transport.u.udp.ser_ports.RTCP);
        break;
...
int RTSP_setup(RTSP_buffer *pRtsp) {
    ...
    if (pRtsp->session_list->rtp_session == NULL) {
        pRtsp->session_list->rtp_session =
             ...
             RTSP_RTP_AVP))) // pStr="RTP/AVP;unicast;client_port=5004-5005"
    {
        ...
        if (!*pStr || (*pStr == ';') || (*pStr == ' ') || (*pStr == '/')) {
                    ...
                    "unicast")) // pStr="unicast;client_port=5004-5005"
            {
                ...
                //服务器端口
                if (RTP_get_port_pair(&Transport.u.udp.ser_ports) !=
                    ERR_NOERROR) {
                ...
                //建立RTP套接字
                rtp_s->hndRtp = (struct _tagStRtpHandle *)RtpCreate(
                    (unsigned int)(((struct sockaddr_in *)(&pRtsp
                                                                ->stClientAddr))
                                       ->sin_addr.s_addr),
                    Transport.u.udp.cli_ports.RTP, _h264nalu);
                printf("<><><><>Creat RTP<><><><>\n");
...
int send_play_reply(RTSP_buffer *pRtsp, RTSP_session *pRtspSessn) {
    ...
    sprintf(
        ...
        RTSP_VER, 200, get_stat(200), pRtsp->rtsp_cseq, PACKAGE, VERSION);
    add_time_stamp(s8Str, 0);
...
int RTSP_play(RTSP_buffer *pRtsp) {
    ...
    if (pRtspSesn != NULL) {
        if (pRtspSesn->session_id == s32SessionId) {
            ...
            for (pRtpSesn = pRtspSesn->rtp_session; pRtpSesn != NULL;
...
void RTSP_state_machine(RTSP_buffer *pRtspBuf, int method) {
    ...
    pRtspSess =
        pRtspBuf
            ->session_list; //如果有连接就会在addclient里里初始化session_list，所以有连接session_list就不会为NULL
    if (pRtspSess == NULL) {
        return;
    }
    ...
    strcat(
        szDebug,
        ((pRtspSess->cur_state == 0)
             ? "init state"
             : ((pRtspSess->cur_state == 1) ? "ready state" : "play state")));
    printf("%s\n", szDebug);
#endif
...
//	trace_point();
    ...
    while (pRtspBuf->in_size) { //根据pRtsp的in_buffer来判断方法类型，出错返回-1
...
// base64_encode2(ringfifo[iput].buffer+off, 4, psp.base64pps, 512);  00 00 00
// 01 68 ce 3c 80
