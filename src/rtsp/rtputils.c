...
**************************************************************************************************/
unsigned int
RtpCreate(unsigned int u32IP, int s32Port, EmRtpPayload emPayload) {
    HndRtp hRtp = NULL;
    ...
    if (0xFF000000 == (u32IP & 0xFF000000)) {
        if (-1 == setsockopt(
                      hRtp->s32Sock, SOL_SOCKET, SO_BROADCAST,
...
static int SendNalu264(HndRtp hRtp, char *pNalBuf, int s32NalBufSize) {
    ...
    /*pSendBuf指向一块大小1500字节的缓冲区
            hRtp->pRtpFixedHdr = (StRtpFixedHdr
       *)pSendBuf;表明hRtp->pRtpFixedHdr指向了pSendBuf的前面12字节 单个NAL包:
            hRtp->pNaluHdr                  = (StNaluHdr *)(pSendBuf +
       12);表明hRtp->pNaluHdr 指向了pSendBuf的第12字节(从0开始计算) pNaluPayload
       = (pSendBuf + 13);表明真正的H264数据存在pSendBuf的第13字节处 FU-A包:
            hRtp->pFuInd            = (StFuIndicator *)(pSendBuf + 12);表明fu
       indicator指向了pSendBuf的第12字节 hRtp->pFuHdr            = (StFuHdr
       *)(pSendBuf + 13);表明fu header位置指向了pSendBuf的第13字节 pNaluPayload
       = (pSendBuf + 14);表明真正的H264数据存在pSendBuf的第14字节处
    */

    hRtp->pRtpFixedHdr = (StRtpFixedHdr *)pSendBuf;
    hRtp->pRtpFixedHdr->u2Version = 2; // RTP协议的版本号

    hRtp->pRtpFixedHdr->u1Marker =
        0; //对视频来说表示标记一帧的结束，后面会被改写为1
    hRtp->pRtpFixedHdr->u7Payload = H264; //有效荷载类型,简单点用96表示无类型

    //计算时间戳
    hRtp->pRtpFixedHdr->u32TimeStamp =
        htonl(hRtp->u32TimeStampCurr * (90000 / 1000)); //时间戳

    hRtp->pRtpFixedHdr->u32SSrc =
        hRtp->u32SSrc; //同步信源(SSRC)标识符,一般填客户机的IP
    ...
    //保存nalu首byte
    u8NaluBytes = *pNalBuf; //注意这里的pNalBuf包含了00 00 00
                            // 01这几个首字节,所以u8NaluBytes=00
    //设置未发送的Nalu数据指针位置
    pNaluCurr = pNalBuf + 1;
    /*这里的pNaluCurr是真正的H264数据的第二个，第一个被赋值给了u8NaluBytes参与FU-A
    indicator与FU-A header 	的计算 如果原来的H264数据为:00 00 00 01 xx xx
    xx...... 那么u8NaluBytes为第一个00 pNaluCurr则为00 00 01 xx xx xx......
    所以后面 memcpy(pNaluPayload, pNaluCurr, s32Bytes)时pNaluPayload也为00 00 01
    67 xx xx xx......这样发送出去后抓包发现 FA-U的第一个包为00 00 01 xx xx
    xx......
    */
    //设置剩余的Nalu数据数量
    ...
    if (s32NaluRemain <= MAX_RTP_PKT_LENGTH) {
        hRtp->pRtpFixedHdr->u1Marker =
            1; //对视频来说表示标记一帧的结束，这里一只一个NALU包，一包即结束，所以为1
        hRtp->pRtpFixedHdr->u16SeqNum =
            htons(hRtp->u16SeqNum++); //标识发送者所发送的RTP报文的序列号
                                      //用网络字节表示
        ...
        /*int sendto ( socket s , const void * msg, int len, unsigned int flags,
           const struct sockaddr * to , int tolen ) ; s 套接字 msg
           待发送数据的缓冲区 len 缓冲区长度 flags 调用方式标志位, 一般为0,
           改变Flags，将会改变Sendto发送的形式 to
           （可选）指针，指向目的套接字的地址 tolen 所指目的地址的长度
                返回:成功则返回实际传送出去的字符数，失败返回－1，错误原因存于errno
           中
        */
        if (sendto(
                hRtp->s32Sock, pSendBuf, s32Bytes, 0,
                (struct sockaddr *)&hRtp->stServAddr,
                sizeof(hRtp->stServAddr)) < 0) {
            s32Ret = -1;
            goto cleanup;
        }
#ifdef SAVE_NALU
    ...
    // NALU包大于最大包长度，分批发送  FA-U模式
        ...
        hRtp->pFuInd->u5Type =
            28; //前面说明了u8NaluBytes=00，计算hRtp->pFuInd=28=
                // 0x1c,抓包发现第一个会有0x1c(抓包的第0x36个)
        ...
        hRtp->pFuHdr->u5Type =
            u8NaluBytes &
            0x1f; //前面说明了u8NaluBytes=00,所以hRtp->pFuHdr->u5Type =0
        ...
        while (s32NaluRemain > 0) {
            ...
            hRtp->pRtpFixedHdr->u1Marker = (s32NaluRemain <= MAX_RTP_PKT_LENGTH)
                                               ? 1
                                               : 0; //最后一个包u1Marker才置1
            ...
            hRtp->pFuHdr->u1E =
                (s32NaluRemain <= MAX_RTP_PKT_LENGTH)
                    ? 1
                    : 0; // u1E:end,代表最后一批，不是最后一批数据则为0
            //第一批数据则置1
            hRtp->pFuHdr->u1S =
                (s32NaluRemain == (s32NalBufSize - 1))
                    ? 1
                    : 0; // u1S:start,代表第一批，第一批数据则为1
            /*
                hRtp->pFuHdr->u5Type    = u8NaluBytes &
        0x1f;//前面说明了u8NaluBytes=00，所以hRtp->pFuHdr->u5Type=0
                hRtp->pFuHdr->u1R       = 0;
        hRtp->pFuHdr->u1E       = (s32NaluRemain <= MAX_RTP_PKT_LENGTH) ? 1 :
        0;//不是最后一批数据则为0 hRtp->pFuHdr->u1S       = (s32NaluRemain ==
        (s32NalBufSize - 1)) ? 1 : 0;//第一批数据则为1
        这样计算起来FA-U包第一个包为10000000=0x80，所以抓包看到0x80(抓包的第0x37个)
                                                        第二个包为00000000=0x00,
        所以抓包看到0x00(抓包的第0x37个)
...
**
**************************************************************************************************/
unsigned int RtpSend(
    unsigned int u32Rtp, char *pData, int s32DataSize,
    ...
    if (_h264 ==
        hRtp->emPayload) //发送H264文件，有多个NALU单元，需要找出00000001来分离NALU
    { // printf("\n\t\th264");
        ...
        for (; pData < pDataEnd - 5; pData++) {
            ...
            if (0x01000000 == u32NaluToken) {
                s32NalSize =
                    (int)(pData - pNalBuf); //二者一相减就是第一个nalu的内容
        ...
        _h264nalu == hRtp->emPayload) //直接发送NALU单元，所以不需要分享NALU
