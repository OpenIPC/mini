...
void send_mp4_to_client(uint8_t chn_index, const void *p) {
    ...
    for (uint32_t i = 0; i < stream->u32PackCount; ++i) {
        ...
        case NalUnitType_SPS: {
            set_sps(&mp4_context, pack_data, pack_len);
            break;
        }
        case NalUnitType_PPS: {
            set_pps(&mp4_context, pack_data, pack_len);
            break;
        }
        case NalUnitType_CodedSliceIdr: {
            set_slice(&mp4_context, pack_data, pack_len, nal.unit_type);
            break;
        }
        ...
        for (uint32_t i = 0; i < MAX_CLIENTS; ++i) {
            ...
            if (!client_fds[i].mp4_state.header_sent) {
                ...
                err = get_header(&mp4_context, &header_buf);
                ...
                client_fds[i].mp4_state.default_sample_duration =
                    default_sample_size;
            }

            err = set_mp4_state(&mp4_context, &client_fds[i].mp4_state);
            chk_err_continue {
                struct BitBuf moof_buf;
                err = get_moof(&mp4_context, &moof_buf);
                chk_err_continue ssize_t len_size =
            ...
            }
            {
                ...
                err = get_mdat(&mp4_context, &mdat_buf);
                chk_err_continue ssize_t len_size =
...
void *send_jpeg_thread(void *vargp) {
    ...
    printf(
        "try to request jpeg (%ux%u, qfactor %u, color2Gray %d) from "
        "hisdk...\n",
        task.width, task.height, task.qfactor, task.color2Gray);
    HI_S32 s32Ret =
        get_jpeg(task.width, task.height, task.qfactor, task.color2Gray, &jpeg);
...
void *server_thread(void *vargp) {
    ...
    while (keepRunning) {
        ...
        if (!parseRequestPath(request_headers, request_path)) {
            close_socket_fd(client_fd);
            continue;
        };
        ...
        if (strcmp(request_path, "./video.mp4") == 0 && app_config.mp4_enable) {
            ...
            for (uint32_t i = 0; i < MAX_CLIENTS; ++i)
                if (client_fds[i].socket_fd < 0) {
                    ...
                    client_fds[i].mp4_state.header_sent = false;
                    break;
                }
            pthread_mutex_unlock(&client_fds_mutex);
            continue;
        ...
        // if mjpeg stream is requested add client_fd socket to client_fds array
        // and send mjpeg stream with http_thread
