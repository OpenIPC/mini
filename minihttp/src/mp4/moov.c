...
enum BufError write_tkhd(struct BitBuf *ptr, const struct MoovInfo *moov_info) {
    ...
    err = put_u32_be(ptr, 125829120); chk_err  // 4 Track width
    err = put_u32_be(ptr, 70778880); chk_err  // 4 Track height
...
enum BufError write_avcC(struct BitBuf *ptr, const struct MoovInfo *moov_info) {
    ...
    err = put_u8(ptr, moov_info->profile_idc); chk_err  // 1 profile
    err = put_u8(ptr, 0); chk_err  // 1 compatibility
    err = put_u8(ptr, moov_info->level_idc); chk_err  // 1 level
    ...
    err = put(ptr, (const char *)moov_info->sps, moov_info->sps_length); chk_err // SPS
    err = put_u8(ptr, 1); chk_err  // 1 num pps
    err = put_u16_be(ptr, moov_info->pps_length); chk_err
    err = put(ptr, (const char *)moov_info->pps, moov_info->pps_length); chk_err // pps
