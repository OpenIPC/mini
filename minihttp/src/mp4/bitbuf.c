...
enum BufError put_skip(struct BitBuf *ptr, const uint32_t count) {
    ...
    if (pos >= ptr->size) return try_to_realloc(ptr, pos);
    ptr->offset = pos;
    return BUF_OK;
}

enum BufError put_to_offset(struct BitBuf *ptr, const uint32_t offset, const char* data, const uint32_t size) {
    chk_ptr
...
enum BufError put_counted_str_to_offset(struct BitBuf *ptr, const uint32_t offset, const char *str, const uint32_t len) {
    ...
    if (pos >= ptr->size) return try_to_realloc(ptr, pos);
    for (uint32_t i = 0; i < len; i++) { ptr->buf[offset + i] = str[i]; }
    ptr->buf[pos] = 0;
    return BUF_OK;
    }
